...
func SettingsHash() string {
	payload := fmt.Sprintf(
		"tape=%s|cost=%s|weights=%s|scalp=false|llm=false|cash=%.0f|name=%.4f|gross=%.4f|cashbuf=%.4f|sector=%.4f|halt=%.4f|stt_buy=%.4f|stt_sell=%.4f|exch=%.6f|sebi=%.6f|stamp=%.4f|gst=%.4f|brokerage=%.2f|adv_base=%.2f|adv_kappa=%.0f|universe=%s",

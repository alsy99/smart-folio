...
- The live roster is a **dated snapshot file** (`data/roster/YYYY-MM-DD.json`, checked in) recording tape, closes, git SHA, admissions, and which shipped defaults **failed the gate** on that tape. Services boot from the latest file, never from an in-memory winner. With no file the desk runs the shipped daily specs and the hero says so: `Roster: default daily specs (no data/roster snapshot)`. Rerun with `go run ./cmd/lab -tape indstocks -write`.
